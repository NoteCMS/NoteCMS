...
export type ContentField = {
  ...
  config?: {
    ...
    /** `entries` field: how values are chosen. */
    mode?: 'manual' | 'latest';
    /** `entries` + latest: how many recent entries (1–50). */
    limit?: number;
    /** `entries` + manual: max selectable entries (1–50). */
    maxItems?: number;
    /** `entries` + latest: sort order for the automatic list. */
